...
import { 
  Star, ShoppingCart, Heart, Share2, ShieldCheck, 
  Truck, Clock, ChevronRight, CheckCircle2 
} from 'lucide-react';
...
export const ProductDetail = () => {
  ...
  useEffect(() => {
    if (id) dataService.getProductById(id).then(res => setProduct(res || null));
  }, [id]);

  if (!product) return (
    <Layout>
      <div className="max-w-7xl mx-auto p-20 text-center">Loading product...</div>
    </Layout>
  );
  ...
  return (
    <Layout>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <Breadcrumbs items={[{ label: 'Shop', path: '/shop' }, { label: product.name }]} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-16">
          ...
          <div className="lg:col-span-6 space-y-4">
            ...
            <div className="flex gap-4">
              {[...Array(3)].map((_, i) => (
                <button key={i} className="w-24 h-24 border-2 border-slate-100 rounded-lg overflow-hidden hover:border-emerald-500 transition-all p-2 bg-white">
          ...
          <div className="lg:col-span-6">
            <div className="flex flex-wrap items-center gap-4 mb-4">
              <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${product.stock > 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                {product.stock > 0 ? 'In Stock' : 'Out of Stock'}
              </span>
              <span className="text-sm font-bold text-slate-400">Brand: <Link to="/shop" className="text-emerald-600 hover:underline">{product.brand}</Link></span>
              <span className="text-sm font-bold text-slate-400">ID: {product.id}</span>
            </div>

            <h1 className="text-3xl font-black text-slate-900 mb-6 leading-tight">{product.name}</h1>
            
            <div className="flex items-center gap-4 mb-8 bg-slate-50 p-6 rounded-2xl border border-slate-100">
              <div>
                <span className="text-4xl font-black text-slate-900">{product.price.toLocaleString()} BDT</span>
                {product.oldPrice && (
                  <div className="flex items-center gap-3">
                    <span className="text-lg text-slate-400 line-through">{product.oldPrice.toLocaleString()} BDT</span>
                    <span className="text-xs font-bold text-rose-600 bg-rose-50 px-2 py-0.5 rounded">SAVE {((1 - product.price/product.oldPrice) * 100).toFixed(0)}%</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-6 mb-8">
              <div className="flex items-center gap-6">
                <div className="flex items-center border border-slate-200 rounded-lg overflow-hidden h-12">
                  <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="px-4 hover:bg-slate-50 transition-colors border-r">-</button>
                  <span className="w-12 text-center font-bold">{quantity}</span>
                  <button onClick={() => setQuantity(q => q + 1)} className="px-4 hover:bg-slate-50 transition-colors border-l">+</button>
                </div>
                <Button 
            ...
            {activeTab === 'specs' && (
              <div className="space-y-0 border border-slate-100 rounded-xl overflow-hidden shadow-sm">
                {Object.entries(product.specs).map(([key, val], idx) => (
                  <div key={key} className={`grid grid-cols-3 p-4 text-sm ${idx % 2 === 0 ? 'bg-slate-50' : 'bg-white'}`}>
